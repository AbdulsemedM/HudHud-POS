                      ...
                      TextFormField(
                          ...
                          validator: (newValue) {
                            if ((newValue == '' || double.parse(newValue!) < 0.01) ||
                                double.parse(newValue) > double.parse(due.toString())) {
      ...
      if (_formKey.currentState!.validate()) {
        if (selectedLocation['id'] != 0) {
          Map<String, dynamic> paymentMap = {
            ...
            "paid_on": DateFormat("yyyy-MM-dd hh:mm:ss").format(DateTime.now()).toString(),
          };
          await ContactPaymentApi().postContactPayment(paymentMap).then((value) {
            Navigator.popUntil(context, ModalRoute.withName('/home'));
            Fluttertoast.showToast(msg: AppLocalizations.of(context).translate('payment_successful'));
          });
        } else {
          ...
          onChanged: (value) async {
            ...
            if (newValue != 0) {
              if (await Helper().checkConnectivity()) {
                ...
                await ContactPaymentApi().getCustomerDue(newValue).then((value) {
                  if (value != null) {

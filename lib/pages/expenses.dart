...
class _ExpenseState extends State<Expense> {
  ...
  Map<String, dynamic> selectedPaymentAccount = {'id': null, 'name': "None"},
      selectedPaymentMethod = {'name': 'name', 'value': 'value', 'account_id': null};
  String symbol = '';
            ...
            children: <Widget>[
                  ...
                  Column(
                    children: <Widget>[
                      Text(
                        ...
                        style:
                            AppTheme.getTextStyle(themeData.textTheme.titleLarge, fontWeight: 700, letterSpacing: -0.2),
                      ),
                      locations(),
                    ],
                  ),
              ...
              Row(
                ...
                children: <Widget>[
                  ...
                  SizedBox(width: MySize.screenWidth! * 0.45, child: expenseCategory()),
                ],
              ),
              Padding(
                padding: EdgeInsets.all(MySize.size8!),
              ...
              Row(
                ...
                children: <Widget>[
                  ...
                  SizedBox(width: MySize.screenWidth! * 0.45, child: expenseSubCategory()),
                ],
              ),
              Padding(
                padding: EdgeInsets.all(MySize.size8!),
                ...
                children: <Widget>[
                  Expanded(
                    child: TextFormField(
                      validator: (value) {
                        if (value!.length < 1) {
                          return AppLocalizations.of(context).translate('please_enter_expense_amount');
                        } else {
                          return null;
                        }
                      },
                      decoration: InputDecoration(
                        prefix: Text(symbol),
                        labelText: AppLocalizations.of(context).translate('expense_amount'),
              ...
              Row(
                children: <Widget>[
                  Expanded(
                    child: TextFormField(
                      decoration: InputDecoration(
                        labelText: AppLocalizations.of(context).translate('expense_note'),
                        border: themeData.inputDecorationTheme.border,
                        enabledBorder: themeData.inputDecorationTheme.border,
                        focusedBorder: themeData.inputDecorationTheme.focusedBorder,
                      ),
              ...
              ElevatedButton(
                ...
                child: Text(
                  ...
                  style: AppTheme.getTextStyle(themeData.textTheme.titleLarge,
                      color: themeData.colorScheme.onPrimary, fontWeight: 700, letterSpacing: -0.2),
                ),
              )
            ],
          ),
  ...
  onSubmit() async {
    if (selectedLocation['id'] != 0) {
      ...
      await ExpenseApi().create(expenseMap).then((value) {
        ...
        Fluttertoast.showToast(msg: AppLocalizations.of(context).translate('expense_added_successfully'));
      });
    } else {
      Fluttertoast.showToast(msg: AppLocalizations.of(context).translate('error_invalid_location'));
    }
  }
  ...
  Widget locations() {
    return PopupMenuButton(
        ...
        child: Container(
          padding:
              EdgeInsets.only(left: MySize.size12!, right: MySize.size12!, top: MySize.size8!, bottom: MySize.size8!),
    ...
    return PopupMenuButton(
      onSelected: (Map<String, dynamic> item) {
        setState(() {
          ...
          selectedExpenseSubCategoryId = {'id': 0, 'name': 'Select'};
          if (item.containsKey('sub_categories') && item['sub_categories'].length > 0) {
  ...
  setExpenseCategories() async {
    await ExpenseApi().get().then((value) {
      value.forEach((element) {
        setState(() {
          expenseCategories
              .add({'id': element['id'], 'name': element['name'], 'sub_categories': element['sub_categories']});
        });
      });
    });
  }
  ...
  setPaymentDetails(int locId) async {
    ...
    List payments = await System().get('payment_method', selectedLocation['id']);
    paymentAccounts = [
      {'id': null, 'name': "None"}
    ];
      ...
      children: <Widget>[
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: <Widget>[
          Expanded(
            child: TextFormField(
                validator: (value) {
                  if (value == '') value = '0.00';
                  if (expenseAmount.text == '' || double.parse(value!) > double.parse(expenseAmount.text)) {
                    return AppLocalizations.of(context).translate('enter_valid_payment_amount');
                  } else {
                    return null;
                  }
                },
                ...
                textAlign: TextAlign.end,
                style: AppTheme.getTextStyle(themeData.textTheme.titleSmall, fontWeight: 400, letterSpacing: -0.2),
                inputFormatters: [FilteringTextInputFormatter(RegExp(r'^(\d+)?\.?\d{0,2}'), allow: true)],
                keyboardType: TextInputType.number,
                onChanged: (value) {}),
          ),
        ...
        Row(children: <Widget>[
          Text(
            ...
            style: AppTheme.getTextStyle(themeData.textTheme.titleLarge, fontWeight: 700, letterSpacing: -0.2),
          ),
          PopupMenuButton(
            onSelected: (item) {
              setState(() {

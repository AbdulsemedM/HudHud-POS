...
class _SalesState extends State<Sales> {
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
            ...
            TextButton(
              onPressed: () async {
                if (await Helper().checkConnectivity()) {
                        ...
                        content: Row(
                          children: [
                            ...
                            Container(
                                ...
                                child: Text(AppLocalizations.of(context).translate('sync_in_progress'))),
                          ],
                        ),
                      );
                    },
                  ...
                  Fluttertoast.showToast(msg: AppLocalizations.of(context).translate('check_connectivity'));
              },
              child: Text(
          ...
          bottom: TabBar(tabs: [
            Tab(icon: Icon(Icons.line_weight), child: Text(AppLocalizations.of(context).translate('recent_sales'))),
            Tab(
  ...
  updateSells(sells) async {
    ...
    Map<String, dynamic> sellMap = Sell().createSellMap(sells, changeReturn, pendingAmount);
                ...
                return Row(
                  ...
                  children: [
                    Text(
                      ...
                      style:
                          AppTheme.getTextStyle(themeData.textTheme.titleMedium, fontWeight: 700, letterSpacing: -0.2),
                    ),
                    Icon(
                      Icons.error_outline,
        ...
        ? ListView.builder(
            ...
            itemBuilder: (context, index) {
              return recentSellItem(
                  price: Helper().formatCurrency(sellList[index]['invoice_amount']),
                  number: sellList[index]['invoice_no'],
                  status: checkStatus(sellList[index]['invoice_amount'], sellList[index]['pending_amount']),
                  time: sellList[index]['transaction_date'],
                  paid: Helper().formatCurrency(sellList[index]['invoice_amount'] - sellList[index]['pending_amount']),
...
//widget for listing sales from api
                              ...
                              children: [
                                      ...
                                      children: [
                                        Text(
                                            (startDateRange != null && endDateRange != null)
                                                ...
                                                : "Date range",
                                            style:
                                                AppTheme.getTextStyle(themeData.textTheme.bodyLarge, fontWeight: 600)),
                                      ],
                                    ),
                                  ),
                                ),
                                ...
                                Row(
                                  children: [
                                    ...
                                    (paymentStatuses.length > 0) ? paymentStatus() : Container()
                                  ],
                                ),
                                Padding(
                                  padding: EdgeInsets.symmetric(vertical: MySize.size6!),
                        ...
                        itemBuilder: (context, index) {
                          if (index == allSalesListMap.length) {
                            ...
                            return allSellItem(
                                ...
                                customerName: allSalesListMap[index]['contact_name'],
                                locationName: allSalesListMap[index]['location_name'],
                                isQuotation: int.parse(allSalesListMap[index]['is_quotation'].toString()));
                          }
                        })
                    : Helper().noDataWidget(context),
              )
  ...
  Widget dateRangePicker() {
    return Scaffold(
      ...
      body: Column(
        children: [
          RangePicker(
            selectedPeriod: selectedPeriod ?? DatePeriod(DateTime.now().subtract(Duration(days: 7)), DateTime.now()),
            onChanged: (DatePeriod newPeriod) {
              setState(() {
                selectedPeriod = newPeriod;
                startDateRange = DateFormat('yyyy-MM-dd').format(newPeriod.start);
                endDateRange = DateFormat('yyyy-MM-dd').format(newPeriod.end);
              });
            },
          ...
          if (startDateRange != null && endDateRange != null)
            Padding(
              padding: const EdgeInsets.all(8.0),
              child: Text(
                "Selected Range: ${DateFormat('yyyy-MM-dd').format(DateTime.parse(startDateRange!))} - ${DateFormat('yyyy-MM-dd').format(DateTime.parse(endDateRange!))}",
                style: TextStyle(fontSize: 16),
              ),
            ),
          // SfDateRangePicker(view: DateRangePickerView.year, selectionMode: DateRangePickerSelectionMode.range, onSelectionChanged: (DateRangePickerSelectionChangedArgs args) {if (args.value.startDate != null) {setState(() {startDateRange = DateFormat('yyyy-MM-dd').format(args.value.startDate).toString();});}if (args.value.endDate != null) {setState(() {endDateRange = DateFormat('yyyy-MM-dd').format(args.value.endDate).toString();});}},),
          Padding(
          ...
          Row(
            ...
            children: [
              ElevatedButton(
                ...
                child: Text(
                  AppLocalizations.of(context).translate('reset'),
                  style: AppTheme.getTextStyle(themeData.textTheme.titleLarge, color: themeData.colorScheme.onPrimary),
                ),
              ),
              ElevatedButton(
                style: ElevatedButton.styleFrom(
  ...
  Widget recentSellItem({number, time, status, price, paid, isSynced, customerName, locationName, isQuotation, index}) {
    //Logic for row items
            ...
            children: <Widget>[
              Text(
                ...
                style: AppTheme.getTextStyle(themeData.textTheme.bodyMedium,
                    fontWeight: 600, letterSpacing: -0.2, color: themeData.colorScheme.onBackground.withAlpha(160)),
              ),
              Row(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: <Widget>[
                  Column(
                    ...
                    children: <Widget>[
                      ...
                      Text(
                        ...
                        style: AppTheme.getTextStyle(themeData.textTheme.bodyMedium, fontWeight: 600, letterSpacing: 0),
                      ),
                      (isQuotation == 0)
                          ? Text(
                ...
                child: Row(
                  children: [
                    ...
                    IconButton(
                        ...
                        onPressed: () async {
                          if (await Helper().checkConnectivity() && sellList[index]['invoice_url'] != null) {
                            final response = await http.Client().get(Uri.parse(sellList[index]['invoice_url']));
                            if (response.statusCode == 200) {
                              await Helper().printDocument(
                                  sellList[index]['id'], sellList[index]['tax_rate_id'], context,
                                  invoice: response.body);
                            } else {
                              await Helper()
                                  .printDocument(sellList[index]['id'], sellList[index]['tax_rate_id'], context);
                            }
                          } else {
                            await Helper()
                                .printDocument(sellList[index]['id'], sellList[index]['tax_rate_id'], context);
                    ...
                    IconButton(
                        ...
                        onPressed: () async {
                          if (await Helper().checkConnectivity() && sellList[index]['invoice_url'] != null) {
                            final response = await http.Client().get(Uri.parse(sellList[index]['invoice_url']));
                            if (response.statusCode == 200) {
                              await Helper().savePdf(sellList[index]['id'], sellList[index]['tax_rate_id'], context,
                    ...
                    ((sellList[index]['pending_amount'] > 0) && canEditSell)
                        ...
                        : Container(),
                    (((sellList[index]['pending_amount'] > 0) && canEditSell) && (sellList[index]['mobile'] != null))
                        ? IconButton(
                            icon: Icon(
                              Icons.call_outlined,
                              color: Colors.green,
                ...
                children: <Widget>[
                  Container(
                    ...
                    decoration: BoxDecoration(
                        ...
                        color: (isQuotation == 0) ? checkStatusColor(status) : Colors.yellowAccent),
                    child: Text(
  ...
  Widget allSellItem({number, time, status, price, paid, isSynced, customerName, locationName, isQuotation, index}) {
    //Logic for row items
    double space = MySize.size12!;
    return Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        (canDeleteSell)
                                      ...
                                      return AlertDialog(
                                        ...
                                        actions: <Widget>[
                                          ...
                                          TextButton(
                                              ...
                                              onPressed: () async {
                                                Navigator.pop(context);
                                                await SellApi().delete(allSalesListMap[index]['id']).then((value) {
                                                  if (value != null) {
                                                    setState(() {
                            ...
                            : Container(),
                        Visibility(
                          visible: allSalesListMap[index]['invoice_url'] != null,
                          child: IconButton(
                              icon: Icon(
                                MdiIcons.printerWireless,
                                color: Colors.deepPurple,
                              ...
                              onPressed: () async {
                                if (await Helper().checkConnectivity()) {
                                  final response =
                                      await http.Client().get(Uri.parse(allSalesListMap[index]['invoice_url']));
                                  if (response.statusCode == 200) {
                          ...
                          child: IconButton(
                              ...
                              onPressed: () async {
                                if (await Helper().checkConnectivity()) {
                                  ...
                                  // print(response.body);
                                  if (response.statusCode == 200) {
                                    await Helper().savePdf(0, 0, context, allSalesListMap[index]['invoice_no'],
                                        invoice: response.body);
                                  } else {
                                    Fluttertoast.showToast(
                                        msg: AppLocalizations.of(context).translate('something_went_wrong'));
            ...
            children: [
              Container(
                padding: EdgeInsets.only(
                    left: MySize.size12!, right: MySize.size12!, top: MySize.size8!, bottom: MySize.size8!),
                decoration: BoxDecoration(
                    borderRadius: BorderRadius.all(Radius.circular(MySize.size4!)),
                    color: (isQuotation == 0) ? checkStatusColor(status) : Colors.yellowAccent),
                child: Text(
        ...
        return DropdownMenuItem<String>(
            ...
            child: Container(
              ...
              child: Text("${value['name']} (${value['mobile'] ?? ' - '})",
                  ...
                  overflow: TextOverflow.ellipsis,
                  style:
                      AppTheme.getTextStyle(themeData.textTheme.bodyMedium, color: themeData.colorScheme.onBackground)),
            ));
      }).toList(),
    ...
    return PopupMenuButton(
      ...
      itemBuilder: (BuildContext context) {
        return paymentStatuses.map((String value) {
          return PopupMenuItem(
            value: value,
            child: Text(AppLocalizations.of(context).translate(value).toUpperCase(),
                style:
                    AppTheme.getTextStyle(themeData.textTheme.bodyMedium, color: themeData.colorScheme.onBackground)),
          );
        }).toList();
      ...
      child: Container(
        ...
        child: Row(
          children: <Widget>[
            Text(
              AppLocalizations.of(context).translate(selectedPaymentStatus).toUpperCase(),
              style: AppTheme.getTextStyle(
                themeData.textTheme.bodyLarge,
                color: themeData.colorScheme.onBackground,
              ),

...
class CheckOutState extends State<CheckOut> {
  ...
  bool _printInvoice = true, printWebInvoice = false, saleCreated = false, isLoading = false;
  ...
  onEdit(sellId) async {
    ...
    await SellDatabase().getSellBySellId(sellId).then((value) {
      ...
      invoiceAmount = argument!['invoiceAmount'] + double.parse(shippingCharges.text);
    });
    payments = [];
    ...
    return Container(
      ...
      child: Column(
        children: <Widget>[
            ...
            child: GestureDetector(
              onTap: () {
                DatePicker.showDateTimePicker(
                  ...
                  currentTime: DateTime.parse(transactionDate),
                  //visitOn
                  // Initial date-time
                  minTime: DateTime.now(),
                  // Minimum selectable date
                  maxTime: DateTime.now().add(const Duration(days: 366)),
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: themeData.colorScheme.primary, width: 1.0),
                ),
                child: Text(
                  transactionDate,
          ...
          // Card(margin: EdgeInsets.all(MySize.size5!), shadowColor: Colors.blue, child: DateTimePicker(use24HourFormat: true, locale: Locale('en', 'US'), initialValue: transactionDate, type: DateTimePickerType.dateTime, firstDate: DateTime.now().subtract(Duration(days: 366)), lastDate: DateTime.now(), dateLabelText: "${AppLocalizations.of(context).translate('date')}:", style: AppTheme.getTextStyle(themeData.textTheme.bodyText1, fontWeight: 700, color: themeData.colorScheme.primary,), textAlign: TextAlign.center, onChanged: (val) {setState(() {transactionDate = val;});},),),
          ListView.builder(
                    ...
                    child: Column(children: <Widget>[
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          Column(
                            children: <Widget>[
                              Text(AppLocalizations.of(context).translate('payment_account') + ' : ',
                                  style: AppTheme.getTextStyle(themeData.textTheme.bodyLarge,
                                      color: themeData.colorScheme.onBackground, fontWeight: 600, muted: true)),
                              DropdownButtonHideUnderline(
                                child: DropdownButton(
                      ...
                      Row(
                        children: <Widget>[
                          SizedBox(
                            ...
                            child: TextFormField(
                                decoration:
                                    InputDecoration(hintText: AppLocalizations.of(context).translate('payment_note')),
                                onChanged: (value) {
                  ...
                  Row(
                    children: [
                      Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
                        Text(AppLocalizations.of(context).translate('shipping_charges') + ' : ',
                            style: AppTheme.getTextStyle(themeData.textTheme.bodyLarge,
                                color: themeData.colorScheme.onBackground, fontWeight: 600, muted: true)),
                        SizedBox(
                            height: MySize.size40,
                        ...
                        SizedBox(
                          ...
                          child: TextFormField(
                              ...
                              decoration:
                                  InputDecoration(hintText: AppLocalizations.of(context).translate('shipping_details')),
                        ...
                        Row(
                          ...
                          children: <Widget>[
                            Column(children: <Widget>[
                              Text(AppLocalizations.of(context).translate('sell_note') + ' : ',
                                  style: AppTheme.getTextStyle(themeData.textTheme.bodyLarge,
                                      color: themeData.colorScheme.onBackground, fontWeight: 600, muted: true)),
                              SizedBox(
                            ...
                            Column(
                              children: <Widget>[
                                Text(AppLocalizations.of(context).translate('staff_note') + ' : ',
                                    style: AppTheme.getTextStyle(themeData.textTheme.bodyLarge,
                                        color: themeData.colorScheme.onBackground, fontWeight: 600, muted: true)),
                                SizedBox(
                                  height: MySize.size80,
  ...
  onSubmit() async {
    ...
    Map<String, dynamic> sell = await Sell().createSell(
        ...
        saleStatus: 'final',
        sellId: sellId,
        shippingCharges: (shippingCharges.text != '') ? double.parse(shippingCharges.text) : 0.00,
      ...
      if (_printInvoice) {
        if (printWebInvoice && invoice != null) {
          ...
          if (response.statusCode == 200) {
            await Helper().printDocument(sellId, argument!['taxId'], context, invoice: response.body).then((value) {
              Navigator.pushNamedAndRemoveUntil(
                  context, (argument!['sellId'] == null) ? '/products' : '/sale', ModalRoute.withName('/home'));
            });
          } else {
            await Helper().printDocument(sellId, argument!['taxId'], context).then((value) {
              Navigator.pushNamedAndRemoveUntil(
                  context, (argument!['sellId'] == null) ? '/products' : '/sale', ModalRoute.withName('/home'));
            });
          }
        ...
        if (printWebInvoice && invoice != null) {
          ...
          if (response.statusCode == 200) {
            ...
            await Helper().savePdf(sellId, argument!['taxId'], context, invoiceNo).then((value) {
              Navigator.pushNamedAndRemoveUntil(
                  context, (argument!['sellId'] == null) ? '/products' : '/sale', ModalRoute.withName('/home'));
            });
          }
  ...
  alertPending(BuildContext context) {
    AlertDialog alert = new AlertDialog(
      content: Text(AppLocalizations.of(context).translate('pending_message'),
          style: AppTheme.getTextStyle(themeData.textTheme.bodyMedium,
              color: themeData.colorScheme.onBackground, fontWeight: 500, muted: true)),
      actions: <Widget>[
        TextButton(
    ...
    AlertDialog alert = new AlertDialog(
      ...
      actions: <Widget>[
        TextButton(
            ...
            child: Text(AppLocalizations.of(context).translate('cancel'))),
        TextButton(
            style: TextButton.styleFrom(foregroundColor: themeData.colorScheme.onError, backgroundColor: Colors.red),
            onPressed: () {
              Navigator.pop(context);

...
class _ShipmentState extends State<Shipment> {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            Padding(
              padding: EdgeInsets.only(left: MySize.size20!, top: MySize.size5!, bottom: MySize.size10!),
              child: Row(
  ...
  Widget locateShipments() {
    return TextButton(
      ...
      child: Text('Map',
          style: AppTheme.getTextStyle(themeData.textTheme.titleMedium, color: themeData.colorScheme.onBackground)),
    );
  }
  ...
  Future<void> _selectDate(BuildContext context) async {
    final DateTime? picked = await showDatePicker(
        context: context, initialDate: selectedDate, firstDate: DateTime(2015), lastDate: DateTime(2101));
  ...
  Future<Map<String, dynamic>> getCustomerNameById(int id) async {
    ...
    Map<String, dynamic> customerName = {'name': customer['name'], 'mobile': customer['mobile']};
    return customerName;
  }
...
// shipment block
  Widget block(int index, int id, {invoiceNo, date, customerName, deliverTo, contactNo, status}) {
    return Card(
                    ...
                    Column(
                      ...
                      children: [
                        ...
                        Visibility(
                          ...
                          visible: (deliverTo.toString().trim() != '' && deliverTo != null),
                        ),
                      ],
                    ),
                    GestureDetector(
                      onTap: () {
                        selectedInlineStatus = shipments[index]['shipping_status'];
                        deliveredToController.text = shipments[index]['delivered_to'] ?? '';
                      ...
                      child: Container(
                          ...
                          decoration: BoxDecoration(
                              boxShadow: [BoxShadow(offset: Offset.zero, color: customAppTheme.bgLayer3)],
                              borderRadius: BorderRadius.all(Radius.circular(MySize.size20!)),
                              border: Border.all(color: customAppTheme.bgLayer3)),
                          child: Text(
    ...
    if (nextPage != null) {
      await dio.get(nextPage!).then((value) {
        ...
        shipment.forEach((element) async {
          Map<String, dynamic> customer = await getCustomerNameById(element['contact_id']);
          setState(() {
            shipments.add({
              'id': element['id'],
              ...
              child: ListView(
                ...
                children: [
                  ListTile(
                    ...
                    subtitle: Text(
                      '$details',
                      style: AppTheme.getTextStyle(themeData.textTheme.bodyLarge, fontWeight: 500),
                    ),
                    isThreeLine: true,
                  ),
                  ListTile(

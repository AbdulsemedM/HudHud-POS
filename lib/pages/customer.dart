...
class _CustomerState extends State<Customer> {
  ...
  Map<String, dynamic> selectedCustomer = {'id': 0, 'name': 'select customer', 'mobile': ' - '};
  ...
  void didChangeDependencies() {
    ...
    if (argument!['customerId'] != null) {
      Future.delayed(Duration(milliseconds: 400), () async {
        await Contact().getCustomerDetailById(argument!['customerId']).then((value) {
          if (this.mounted) {
            setState(() {
              selectedCustomer = {'id': argument!['customerId'], 'name': value['name'], 'mobile': value['mobile']};
            });
          }
        });
      });
    ...
    return Scaffold(
        ...
        body: SingleChildScrollView(
          child: Column(
            children: <Widget>[
              Padding(
                padding: EdgeInsets.only(top: MySize.size120!, left: MySize.size20!),
                child: Card(child: customerList()),
              ),
              Center(
                child: Visibility(
                  visible: (selectedCustomer['id'] == 0),
                  child: Text(AppLocalizations.of(context).translate('please_select_a_customer_for_checkout_option'),
        ...
        bottomNavigationBar: Visibility(
          ...
          child: Row(
            mainAxisAlignment:
                (argument!['is_quotation'] == null) ? MainAxisAlignment.spaceAround : MainAxisAlignment.center,
            children: [
              Visibility(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Text(
                        ...
                        style: AppTheme.getTextStyle(Theme.of(context).textTheme.bodyLarge,
                            color: Theme.of(context).colorScheme.surface),
                      ),
                    ],
                  ),
              ...
              onPressed: () async {
                if (argument!['sellId'] != null) {
                  ...
                  await SellDatabase().storeSell(sell).then((value) async {
                    ...
                    Navigator.pushNamedAndRemoveUntil(context, '/products', ModalRoute.withName('/home'));
                  });
                }
              },
              ...
              onPressed: () async {
                await SellDatabase().storeSell(sell).then((value) async {
                  ...
                  Helper().printDocument(value, argument!['taxId'], context).then((value) {
                    Navigator.pushNamedAndRemoveUntil(context, '/products', ModalRoute.withName('/home'));
                    Fluttertoast.showToast(msg: AppLocalizations.of(context).translate('quotation_added'));
                  });
                });
              },
  ...
  selectCustomer() async {
    ...
    customers.forEach((value) {
      setState(() {
        customerListMap.add({'id': value['id'], 'name': value['name'], 'mobile': value['mobile']});
      });
      if (value['name'] == 'Walk-In Customer') {
        selectedCustomer = {'id': value['id'], 'name': value['name'], 'mobile': value['mobile']};
      }
    });
  }
}

...
class _NewVisitFormState extends State<NewVisitForm> {
  ...
  String visitOn = DateFormat('yyyy-MM-dd HH:mm:ss').format(DateTime.now());
                    ...
                    children: <Widget>[
                      ...
                      Column(
                        ...
                        children: [
                            ...
                            child: GestureDetector(
                              onTap: () {
                                DatePicker.showDateTimePicker(
                                  ...
                                  currentTime: DateTime.parse(visitOn),
                                  //visitOn
                                  // Initial date-time
                                  ...
                                  onConfirm: (date) {
                                    setState(() {
                                      visitOn = DateFormat('yyyy-MM-dd HH:mm:ss').format(date);
                                    });
                                  },
                                );
                              },
                          ...
                          // Card(margin: EdgeInsets.all(MySize.size5!), child: DateTimePicker(use24HourFormat: false, locale: Locale('en', 'US'), initialValue: visitOn, type: DateTimePickerType.dateTime, firstDate: DateTime.now(), lastDate: DateTime.now().add(Duration(days: 366)), dateMask: 'yyyy-MM-dd  hh:mm', style: AppTheme.getTextStyle(themeData.textTheme.bodyText1, fontWeight: 700, color: themeData.colorScheme.primary,), onChanged: (val) {setState(() {visitOn = val;});},),),
                        ],
                      ),
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        ...
                        child: TextButton(
                          ...
                          onPressed: () async {
                            ...
                            if (fromContact && selectedCustomer['id'] == 0) {
                              validated = false;
                              Fluttertoast.showToast(msg: AppLocalizations.of(context).translate('please_set_contact'));
                            }
                            ...
                            if (await Helper().checkConnectivity()) {
                              if (_formKey.currentState!.validate() && validated) {
                                ...
                                Map visitDetails = {
                                  if (fromContact == true) 'contact_id': selectedCustomer['id'],
                                  if (fromContact == false) 'visit_to': nameController.text,
                                  if (fromContact == false) 'visit_address': addressController.text,
                                  'assigned_to': Config.userId,
                                  'visit_on': DateFormat('yyyy-MM-dd HH:mm:ss').format(DateTime.now()),
                                  'visit_for': visitForController.text
                                };
                                FieldForceApi().create(visitDetails).then((value) {
                                  if (value != null) {
                                    Fluttertoast.showToast(
                                        msg: AppLocalizations.of(context).translate('status_updated'));
                                  }
  ...
  setCustomerList() async {
    ...
    await Contact().get().then((value) {
      value.forEach((Map<String, dynamic> element) {
        setState(() {
          customerListMap.add({'id': element['id'], 'name': element['name'], 'mobile': element['mobile']});
        });
      });
    });
  }
